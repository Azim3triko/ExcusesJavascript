...
window.onload = function() {
  ...
  let object = ["my homework", "the keys", "the car", "the book"];
  let when = [
    "before the class",
    "right on time",
    "when I finished",
  ...
  let objectrandom = object[Math.floor(Math.random() * object.length)];
  let whenrandom = when[Math.floor(Math.random() * when.length)];

  document.querySelector("#excuse").innerHTML =
    whorandom + " " + actionrandom + " " + objectrandom + " " + whenrandom;
};
